//
//  ZXWaitingPayDetailController.m
//  PocketOrder
//
...
@interface ZXWaitingPayDetailController ()<UIScrollViewDelegate>
...
@implementation ZXWaitingPayDetailController
...
{
    [self addChildViewController:[[ZXOrderDetailController alloc] init]];
    if ([self.title isEqualToString:@"待付款"]) {
        [self addChildViewController:[[ZXOrderStatusController alloc] init]];
    }
    if ([self.title isEqualToString:@"已支付"]) {
        [self addChildViewController:[[ZXPaidOrderStatusController alloc] init]];
    }
    if ([self.title isEqualToString:@"待评价"]) {
        [self addChildViewController:[[ZXWaitEvaluatedOrderStatusController alloc] init]];
    }
    if ([self.title isEqualToString:@"已评价"]) {
        [self addChildViewController:[[ZXEvaluatedOrderStatusController alloc] init]];
    }
}
...
- (void)setupScrollView
{
    ...
    scrollView.frame = CGRectMake(0, SZXNavMaxY + SZXTitlesViewH, ScreenW, ScreenH - (SZXTitlesViewH + SZXNavMaxY));
